...
export default function AnimePage() {
  ...
  const animeList = contents.filter(item => item.type === 'anime');
  const filteredList = selectedCategory === 'Tümü' 
    ? animeList 
    : animeList.filter(item => item.category === selectedCategory);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col relative overflow-hidden">
      ...
      <main className="max-w-6xl mx-auto px-4 py-10 w-full space-y-8">
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-black text-white">Animeler</h1>
          <p className="text-sm text-slate-400">Yayınlanan tüm anime serileri ve bölümleri.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filteredList.length > 0 ? (
            filteredList.map((item) => (
              <Link href={`/watch/${item.id}`} key={item.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-rose-500 transition group block">
                <div className="aspect-video bg-slate-950 flex items-center justify-center relative overflow-hidden">
                  {item.coverUrl ? (
                    ...
                    <Play size={40} className="text-rose-500 group-hover:scale-110 transition" />
                  )}
                ...
                <div className="p-4 space-y-2">
                  ...
                  <p className="text-xs text-slate-400">Yazar: {item.author}</p>
            ...
            <div className="col-span-full text-center py-20 text-slate-500">Bu kategoride henüz anime bulunmuyor.</div>
          )}
        </div>
      </main>
    </div>
