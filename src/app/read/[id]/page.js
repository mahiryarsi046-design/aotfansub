...
export default function ReadPage() {
  ...
  const seriesEpisodes = allContents
    .filter(item => item.title.toLowerCase() === content.title.toLowerCase() && item.type === 'manga')
  ...
  return (
    <div className="min-h-screen text-slate-100 flex flex-col relative overflow-hidden bg-slate-950 bg-[url('/aot-bg.jpg')] bg-cover bg-center bg-fixed">
      ...
      <main className="max-w-3xl mx-auto px-4 py-10 w-full space-y-6 flex-grow">
        <div className="flex items-center justify-between bg-slate-900/90 backdrop-blur border border-slate-800 p-4 rounded-2xl">
          ...
          <div className="flex items-center gap-2">
            {prevEpisode && (
              <Link href={`/read/${prevEpisode.id}`} className="bg-slate-800 hover:bg-slate-700 text-white px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-1 transition">
                <ChevronLeft size={16} /> Önceki
              </Link>
            )}
            {nextEpisode && (
              <Link href={`/read/${nextEpisode.id}`} className="bg-rose-600 hover:bg-rose-700 text-white px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-1 transition">
        ...
        {/* Manga Sayfaları veya Kilit Ekranı */}
        <div className="bg-slate-900/90 backdrop-blur border border-slate-800 rounded-2xl p-6 shadow-2xl flex flex-col items-center">
          {unlocked ? (
            content.mangaImages && content.mangaImages.length > 0 ? (
              <div className="w-full space-y-4 flex flex-col items-center">
                {content.mangaImages.map((imgSrc, idx) => (
                  <img key={idx} src={imgSrc} alt={`Sayfa ${idx + 1}`} className="max-w-full rounded shadow-lg" />
                ))}
              </div>
            ) : (
              <div className="text-slate-400 py-20">Manga görselleri bulunamadı.</div>
            )
          ) : (
